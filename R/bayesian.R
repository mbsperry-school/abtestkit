...
elicit_beta_prior <- function(guess, worst_case, best_case) {
  ...
  if (use_moment) {
    ...
    objective <- function(params) {
      ...
      q_low <- stats::qbeta(0.025, a, b)
      q_high <- stats::qbeta(0.975, a, b)
      (q_low - worst_case)^2 + (q_high - best_case)^2
    }

    fit <- stats::optim(start, objective,
      method = "L-BFGS-B",
      lower = c(0.001, 0.001)
    )
    alpha <- fit$par[1]
...
bayes_ab_summary <- function(posterior_control, posterior_treatment,
  ...
  ci_a <- stats::quantile(draws_control, probs = c(0.025, 0.975))
  ci_b <- stats::quantile(draws_treatment, probs = c(0.025, 0.975))
  ci_lift <- stats::quantile(lift, probs = c(0.025, 0.975))
